import { BitcoinAPI, sortUTXOs, UTXO } from "./apiUtils";
...
export class Blockchair implements BitcoinAPI {
      ...
      public fetchUTXO = async (txid: string, txindex: string): Promise<UTXO> => {
            ...
            const response = await GET<TransactionResponse>(`${url}`);
            ...
            if (latestBlock === 0) {
                  ...
                  const statsResponse = await GET<{ data: { blocks: number } }>(statsUrl);
      ...
      public fetchUTXOs = async (address: string): Promise<UTXO[]> => {
            ...
            const response = await GET<AddressResponse>(url);
            ...
            return response.data[address].utxo
                  .map((utxo) => ({
                        ...
                        height: utxo.block_id && utxo.block_id > 0 ? utxo.block_id.toString() : null,
                  }))
                  .sort(sortUTXOs);
      };
      ...
      public fetchTXs = async (address: string, limit = 25): Promise<UTXO[]> => {
            ...
            for (let i = 0; i < Math.ceil(txHashes.length / 10); i++) {
                  const txUrl = `${this.endpoint()}/dashboards/transactions/${txHashes
                        .slice(i * 10, (i + 1) * 10)
                        .join(",")}`;
                  const txResponse = await GET<TransactionResponse>(txUrl);
                  txDetails = {
                        ...txDetails,
                        ...txResponse.data,
                  };
      ...
      public broadcastTransaction = async (txHex: string): Promise<string> => {
            ...
            if ((response.data as unknown as BlockchairError).error) {
                  throw new Error((response.data as unknown as BlockchairError).error);
            }
