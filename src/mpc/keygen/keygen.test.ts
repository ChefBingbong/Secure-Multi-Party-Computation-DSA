...
describe("keygen 2/3", async () => {
      ...
      test("initiate session", async () => {
            ...
            sessionA = new KeygenSession(
                  "a",
                  partyIds,
                  threshold
                  // precomputedPaillierPrimesA
            );
            sessionB = new KeygenSession(
                  "b",
                  partyIds,
                  threshold
                  // precomputedPaillierPrimesB
            );
            sessionC = new KeygenSession(
                  "c",
                  partyIds,
                  threshold
                  // precomputedPaillierPrimesC
            );
      });

      test("round 1", async () => {
            const keygenRound1A = new KeygenRound1(
                  sessionA,
                  sessionA.inputForRound1
            );
            outputRound1A = await keygenRound1A.process();

            const keygenRound1B = new KeygenRound1(
                  sessionB,
                  sessionB.inputForRound1
            );
            outputRound1B = await keygenRound1B.process();

            const keygenRound1C = new KeygenRound1(
                  sessionC,
                  sessionC.inputForRound1
            );
      ...
      test("round 2", async () => {
            const allBroadcasts = [
                  ...
                  .map((b) => b.toJSON())
                  .map((b) => KeygenBroadcastForRound2.fromJSON(b));
            assert.equal(allBroadcasts.length, 3);

            const keygenRound2A = new KeygenRound2(
                  sessionA,
                  outputRound1A.inputForRound2
            );
            allBroadcasts.forEach((b) => keygenRound2A.handleBroadcastMessage(b));
            outputRound2A = keygenRound2A.process();

            const keygenRound2B = new KeygenRound2(
                  sessionB,
                  outputRound1B.inputForRound2
            );
            allBroadcasts.forEach((b) => keygenRound2B.handleBroadcastMessage(b));
            outputRound2B = keygenRound2B.process();

            const keygenRound2C = new KeygenRound2(
                  sessionC,
                  outputRound1C.inputForRound2
            );
      ...
      test("round 3", async () => {
            const allBroadcasts = [
                  ...
                  .map((b) => b.toJSON())
                  .map((b) => KeygenBroadcastForRound3.fromJSON(b));
            assert.equal(allBroadcasts.length, 3);

            const keygenRound3A = new KeygenRound3(
                  sessionA,
                  outputRound2A.inputForRound3
            );
            allBroadcasts.forEach((b) => keygenRound3A.handleBroadcastMessage(b));
            outputRound3A = keygenRound3A.process();

            const keygenRound3B = new KeygenRound3(
                  sessionB,
                  outputRound2B.inputForRound3
            );
            allBroadcasts.forEach((b) => keygenRound3B.handleBroadcastMessage(b));
            outputRound3B = keygenRound3B.process();

            const keygenRound3C = new KeygenRound3(
                  sessionC,
                  outputRound2C.inputForRound3
            );
      ...
      test("round 4", async () => {
            const allBroadcasts = [
                  ...
                  .map((b) => b.toJSON())
                  .map((b) => KeygenBroadcastForRound4.fromJSON(b));
            assert.equal(allBroadcasts.length, 3);
            const allMessages = [
                  ...
                  .map((m) => m.toJSON())
                  .map((m) => KeygenDirectMessageForRound4.fromJSON(m));
            assert.equal(allMessages.length, 6);

            const keygenRound4A = new KeygenRound4(
                  sessionA,
                  outputRound3A.inputForRound4
            );
            ...
            const keygenRound4B = new KeygenRound4(
                  sessionB,
                  outputRound3B.inputForRound4
            );
            ...
            const keygenRound4C = new KeygenRound4(
                  sessionC,
                  outputRound3C.inputForRound4
            );
