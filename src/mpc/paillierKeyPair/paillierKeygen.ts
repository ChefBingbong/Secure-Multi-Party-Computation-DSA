...
import { modInv, gcd, abs, modPow, modMultiply, randBetween } from "bigint-crypto-utils";

import { sampleUnitModN } from "../math/sample";
import { JSONable, PedersenParams } from "../paillierKeyPair/Pedersen/pendersen";
...
export class PaillierSecretKey implements JSONable {
      ...
      public constructor(p: bigint, q: bigint, phi: bigint, phiInv: bigint, publicKey: PaillierPublicKey) {
            this._p = p;
            this._q = q;
            this._phi = phi;
            this._phiInv = phiInv;
...
export class PaillierPublicKey implements Hashable, JSONable {
      ...
      public constructor(n: bigint, nSquared: bigint, nPlusOne: bigint) {
            this._n = n;
      ...
      public get nSquared(): bigint {
            return this._nSquared;
      }
      public get nPlusOne(): bigint {
            return this._nPlusOne;
      }
...
export const paillierAdd = (publicKey: PaillierPublicKey, ciphertextA: bigint, ciphertextB: bigint): bigint => {
      const ciphertextSum = modMultiply([ciphertextA, ciphertextB], publicKey.nSquared);
      return ciphertextSum;
};

export const paillierMultiply = (publicKey: PaillierPublicKey, ciphertext: bigint, scalar: bigint): bigint => {
      const ciphertextProduct = modPow(ciphertext, scalar, publicKey.nSquared);
      return ciphertextProduct;
};

const samplePedersen = (
      phi: bigint,
      n: bigint
): {
      s: bigint;
