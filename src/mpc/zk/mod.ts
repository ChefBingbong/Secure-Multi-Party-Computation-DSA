...
export const zkModIsProofValid = (
      ...
      if (jacobi(proof.W, pub.N) !== -1) {
            return false;
      }
      if (!isValidModN(pub.N, proof.W)) {
            return false;
      }
      for (const r of proof.Responses) {
            if (!isValidModN(pub.N, r.X)) {
                  return false;
            }
            if (!isValidModN(pub.N, r.Z)) {
                  return false;
            }
      }
      return true;
...
export const zkModCreateProof = (
      ...
      const rs: ZkModResponse[] = ys.map((y) => {
            ...
            const {
                  ...
                  b: B,
                  out: yPrime,
            } = zkModMakeQuadraticResidue(y, w, pHalf, qHalf, n, p, q);
...
const verifyResponse = (
      ...
      let rhs = y;
      if (r.A) {
            rhs = modMultiply([rhs, -1n], n);
      }
      if (r.B) {
            rhs = modMultiply([rhs, w], n);
      }
...
export const zkModVerifyProof = async (
      ...
      if (await isProbablyPrime(n, 20)) {
            return false;
      }

      if (jacobi(proof.W, n) !== -1) {
            return false;
      }

      if (!isValidModN(n, proof.W)) {
            return false;
      }
