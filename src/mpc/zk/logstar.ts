...
export class ZkLogstarProof {
      ...
      private constructor(commitment: ZkLogstarCommitment, Z1: bigint, Z2: bigint, Z3: bigint) {
...
export const zkLogstarIsProofValid = (proof: ZkLogstarProof, pub: ZkLogstarPublic): boolean => {
      if (!proof) {
            return false;
      }
...
export const zkLogstarVerifyProof = (proof: ZkLogstarProof, pubIn: ZkLogstarPublic, hasher: Hasher): boolean => {
      const pub = {
            ...pubIn,
            G: pubIn.G ?? secp256k1.ProjectivePoint.BASE.toAffine(),
      };

      if (!zkLogstarIsProofValid(proof, pubIn)) {
            return false;
      }
      ...
      const rhs = paillierAdd(pub.prover, paillierMultiply(pub.prover, pub.C, e), proof.commitment.A);

      return lhs === rhs;
};

const challenge = (pub: ZkLogstarPublic, commitment: ZkLogstarCommitment, hasher: Hasher): bigint => {
