...
const isValid = (proof: ZkAffpProof, pub: ZkAffpPublic): boolean => {
      ...
      if (!pub.prover.validateCiphertext(proof.commitment.Bx)) {
            return false;
      }
      if (!pub.prover.validateCiphertext(proof.commitment.By)) {
            return false;
      }
      if (!isValidModN(pub.prover.n, proof.Wx)) {
            return false;
      }
...
export const zkAffpCreateProof = (pub: ZkAffpPublic, priv: ZkAffpPrivate, hasher: Hasher): ZkAffpProof => {
...
export const zkAffpVerifyProof = (proof: ZkAffpProof, pub: ZkAffpPublic, hasher: Hasher): boolean => {
      if (!isValid(proof, pub)) {
            return false;
      }
...

      {
            ...
            const rhs = paillierAdd(pub.prover, paillierMultiply(pub.prover, pub.Xp, e), proof.commitment.Bx);
            if (lhs !== rhs) {
                  return false;
            }
      }
...
const challenge = (pub: ZkAffpPublic, commitment: ZkAffpCommitment, hasher: Hasher): bigint => {
